...
bool IsValidName(const std::string& content, const unsigned int maxSize)
{
    if (content.empty() || content.length() > maxSize) {
        return false;
    }
...
{
    HiLog::Debug(LABEL, "eventbase result code is %{public}d", retCode_);
    return retCode_;
}

void InnerWriter::EventBase::SetRetCode(int retCode)
{
    HiLog::Debug(LABEL, "eventbase set result code to be %{public}d", retCode);
    retCode_ = retCode;
}

size_t InnerWriter::EventBase::GetParamCnt()
{
    HiLog::Debug(LABEL, "count of appended parameter(s) is %{public}zu", builder_.GetParamCnt());
    return builder_.GetParamCnt();
}

RawData& InnerWriter::EventBase::GetEventRawData()
{
...
{
    HiLog::Debug(LABEL, "length of appended string is %{public}zu", value.length());
...
{
    HiLog::Debug(LABEL, "size of appended array is %{public}zu", size);
...
{
    if (!IsValidName(eventBase.GetEventBuilder().GetDomain(), MAX_DOMAIN_LENGTH)) {
        eventBase.SetRetCode(ERR_DOMAIN_NAME_INVALID);
        return;
    }
    if (!IsValidName(eventBase.GetEventBuilder().GetName(), MAX_EVENT_NAME_LENGTH)) {
...
void InnerWriter::AppendStringParam(EventBase& eventBase, const HiSysEventParam& param)
{
    ...
    if (CheckParamValidity(eventBase, param.name)) {
        ...
        eventBase.GetEventBuilder().AppendValue(std::make_shared<StringEncodedParam>(param.name,
            std::string(param.v.s)));
    }
}
