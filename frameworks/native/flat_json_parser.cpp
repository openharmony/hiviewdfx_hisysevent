...
{
    for (char c = '0'; c <= '9'; c++) {
        charFilter_[static_cast<int>(c)] = NUMBER_FLAG;
    }
    charFilter_[static_cast<int>('-')] = NUMBER_FLAG;
    charFilter_[static_cast<int>('+')] = NUMBER_FLAG;
    charFilter_[static_cast<int>('.')] = NUMBER_FLAG;
    charFilter_[static_cast<int>('"')] = STRING_FLAG;
    charFilter_[static_cast<int>('{')] = BRACKET_FLAG;
    charFilter_[static_cast<int>('[')] = BRACKET_FLAG;
}
...
void FlatJsonParser::Parse(const std::string& jsonStr)
{
    ...
    while (index_ < jsonStr.length()) {
        if (charFilter_[static_cast<int>(jsonStr[index_])] != STRING_FLAG) {
            ++index_;
            continue;
        }
...
std::string FlatJsonParser::Print(PrintKvHandler handler)
{
    std::string jsonStr = "{";
    if (!kvList_.empty()) {
        for (size_t i = 0; i < kvList_.size() - 1; i++) {
            jsonStr += (handler(kvList_[i]) + ",");
        }
        jsonStr += handler(kvList_.back());
    }
    jsonStr += "}";
    return jsonStr;
}
...
std::string FlatJsonParser::ParseValue(const std::string& jsonStr)
{
    ...
    while (index_ < jsonStr.length()) {
        int charCode = static_cast<int>(jsonStr[index_]);
...
std::string FlatJsonParser::ParseNumer(const std::string& jsonStr)
{
    std::string number;
    while (index_ < jsonStr.length()) {
        if (charFilter_[static_cast<int>(jsonStr[index_])] != NUMBER_FLAG) {
            break;
        }
